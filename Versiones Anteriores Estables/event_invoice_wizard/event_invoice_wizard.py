        ...
        for reg in all_regs:   
            try:
                ...
                if self.importe == 'inscripcion' and reg.estado == 'pendiente' and reg.state == 'open':
                    ...
                    concepto_linea = "Adelanto por reserva %s" %str(reg.event_id.name)
            ...
            if fact.causa_factura == 'inscripcion':
                if cont == 1:
                    if (primer_reg.alumno_en_factura):
                        ...
                        texto_linea = 'Adelanto por reserva %s para %s'% (str(primer_reg.event_id.name),nombre_alumno)
                    else:
                        texto_linea = 'Adelanto por reserva %s'% str(primer_reg.event_id.name)
                else:
                    texto_linea = 'Adelanto por reserva %s para %d alumnos'% (str(primer_reg.event_id.name), cont)
                idlinef = line_obj.create({'invoice_id' : idf.id,
                                       'name' : texto_linea,
