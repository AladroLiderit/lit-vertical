...
class CarWorkshop(models.Model):
    ...
    vehicle_id = fields.Many2one('car.car', string='Vehicle', track_visibility='onchange',required=True)
    ...
    note = fields.Text('Nota para presupuesto')
    ...
    date_deadline = fields.Datetime(string='Deadline', select=True, copy=False, required=True)
    ...
    amount_total = fields.Float(string='Total Amount Net', readonly=True, compute="amount_total1")
    amount_taxes = fields.Float(string='Total Taxes', readonly=True, compute="_amount_taxes")
    amount_total_tax = fields.Float(string='Total Amount with Taxes', readonly=True, compute="amount_total_taxes")
    ...
    def action_view_invoice(self, cr, uid, ids, context=None):
        ...
        for so in self.browse(cr, uid, ids, context=context):
            inv_ids.append(so.invoice_id.id)
    ...
    @api.depends('planned_works.work_cost', 'materials_used.price', 
        'materials_used.discount','materials_used.amount')
        ...
        for records in self:
            for hour in records:
                ...
                for line2 in hour.materials_used:
                    amount_totall += line2.price *(1-(line2.discount/100))*line2.amount
                records.amount_total = amount_totall


    @api.depends('planned_works.work_cost', 'materials_used.price')
    def _amount_taxes(self):
        for records in self:
            for hour in records:
                amount_totall = 0.0
                for line in hour.planned_works:
                    for t in line.tax_ids:
                        amount_totall += line.work_cost * t.amount
                for line2 in hour.materials_used:
                    for t2 in line2.tax_ids:
                        amount_totall += line2.price *(1-(line2.discount/100))*line2.amount * t2.amount
                records.amount_taxes = amount_totall


    @api.depends('amount_total', 'amount_taxes')
    def amount_total_taxes(self):
        for records in self:
            self.amount_total_tax = self.amount_total + self.amount_taxes
    ...
    def workshop_create_invoices(self):
        ...
        inv_data = {
            ...
            'origin': self.vehicle_id.name.name,
        ...
        for records in self.materials_used:
            line_name = records.description
            if records.material.id:
                line_product = records.material.id
                income_account_line = records.material.property_account_income.id
                if records.material.income_analytic_account_id:
                    line_analytic = records.material.income_analytic_account_id.id
                else:
                    if records.material.categ_id.income_analytic_account_id:
                        line_analytic = records.material.categ_id.income_analytic_account_id.id
                    else:
                        line_analytic = False
            else:
                line_product = False
                income_account_line = self.env['ir.property'].get('property_account_income_categ', 'product.category').id
                line_analytic = False


            if not income_account_line:
                income_account_line = records.material.categ_id.property_account_income_categ.id
            if not income_account_line:
                raise osv.except_osv(_('UserError!'), _('There is no income account defined '
                                                        'for this product: "%s".') % (records.material.name,))
            taxes = records.tax_ids or False
            if fiscal_position:
            ...
            tax_lines =[]
            for t in taxes:
                tax_lines.append(t.id)


            inv_line_data = {
                'name': line_name,
                'account_id': income_account_line,
                'price_unit': records.price,
                'discount':records.discount,
                'quantity': records.amount,
                'product_id': line_product,
                'uos_id': 1,
                'invoice_line_tax_id': [(6, 0,tax_lines)],
                'account_analytic_id': line_analytic,
        ...
        for records in self.planned_works:
            ...
            if not income_account:
                income_account = records.planned_work.categ_id.property_account_income_categ.id
            if not income_account:
                raise osv.except_osv(_('UserError!'), _('There is no income account defined '
                                                        'for this product: "%s".') % (records.planned_work.name,))
            if records.planned_work.income_analytic_account_id:
                line_analytic = records.planned_work.income_analytic_account_id.id
            else:
                if records.planned_work.categ_id.income_analytic_account_id:
                    line_analytic = records.planned_work.categ_id.income_analytic_account_id.id
                else:
                    line_analytic = False
            ...
            inv_line_data = {
                ...
                'price_unit': records.work_price,
                'discount':records.work_discount,
                'quantity': records.time_spent,
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.get('vehicle_id') and not context.get('default_vehicle_id'):
            ...
            vehicle = self.pool.get('fleet.vehicle').browse(cr, uid, vals.get('vehicle_id'), context=context)
            vals['partner_id'] = vehicle.driver_id.id
        if vals.get('user_id'):
    ...
    def onchange_vehicle(self, cr, uid, id, vehicle_id, context=None):
        ...
        if vehicle_id:
            ...
            if vehicle.exists():
                ...
                values['partner_id'] = vehicle.driver_id.id
