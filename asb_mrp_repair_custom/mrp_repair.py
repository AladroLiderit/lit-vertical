...
class mrp_repair(models.Model):
    ...
    journal_id = fields.Many2one('account.journal', string='Diario Factura', default = 1)
    ...
    def _onchange_lot_id(self):
        if self.lot_id:
            ...
            st_lot = self.env['stock.move'].search([('restrict_lot_id','=',self.lot_id.id),('partner_id','!=',False)])
    ...
    def action_repair_done(self, cr, uid, ids, context=None):
        ...
        for repair in self.browse(cr, uid, ids, context=context):
            ...
            vals_order['num_pedido'] = repair.num_pedido
            vals_order['destino'] = repair.destino
            vals_order['att'] = repair.att
            vals_order['client_order_ref'] = repair.referencia
            ...
            concept = "REPARACION "+ repair.product_id.name
            if repair.lot_id:
                    concept += " "+repair.lot_id.name
            ...
            if group_id:
                ...
                if pick_repair:
                    ...
                    # paramos la confirmacion del picking para que se pueda modificar el pedido de la reparacion
    ...
    def action_invoice_create(self, group=False):
        ...
        for repair in self:
            if repair.picking_out:
                ...
                # si se hace un cambio en el pedido de venta, el picking puede estar ahora cancelado
                if pick_out.state == 'cancel':
                    logger.error('Invoice create detectado albaran cancelado %s'%pick_out.id)
                    sale_pick = self.env['sale.order'].search([('id','=',repair.sale_id.id)])
                    new_pick_out = self.env['stock.picking'].search([
                        ('invoice_state','=','2binvoiced'),
                        ('origin', '=', sale_pick.origin)
                        ])
                    if new_pick_out:
                        self.picking_out = new_pick_out[0].id
                        pick_out = self.env['stock.picking'].browse(new_pick_out[0].id)
                        logger.error('Invoice create albaran sustitudo del cancelado %s'%pick_out.id)
                    else:
                        raise exceptions.Warning(
                    _("No se localiza albaran valido para la factura."))
                
                #si no esta transferido, transferirlo primero
                logger.error('Invoice create estado del albaran %s'%pick_out.state)
                if pick_out.state not in ('cancel','done'):
                    pick_out.action_done()

                #facturar albaran
                rep_journal_id = repair.journal_id.id
