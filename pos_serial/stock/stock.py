...
class stock_production_lot(osv.osv):
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None, context={}, count=False):
        ret_val = super(stock_production_lot, self).search(cr, uid, args, offset=offset, limit=limit, order=order,
                                                      context=context, count=count)
        if context and context.get('location_id'):
            new_val = []
            for eachlot in self.browse(cr, uid, ret_val):
                for quant in eachlot.quant_ids:
                    if context.get('location_id') == quant.location_id.id:
                        new_val.append(eachlot.id)
            return new_val
        return ret_val
...
class stock_picking(osv.osv):
    ...
    def _prepare_pack_ops(self, cr, uid, picking, quants, forced_qties, context=None):
        ...
        # Do the same for the forced quantities using move lines (in cases of force_assign or incomming shipment for example)
        if picking.sale_id != False:
            ...
            for move in picking.move_lines:
                ...
                key = (move.product_id.id, False, move.restrict_lot_id.id, picking.owner_id.id, picking.location_id.id, suggested_location_id)
                if qtys_grouped.get(key):
