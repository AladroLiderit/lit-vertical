...
class event_event(models.Model):
    ...
    @api.depends('precio_total_event', 'descuento_event', 'precio_reserva_event')
    @api.onchange('precio_total_event', 'descuento_event', 'precio_reserva_event')
    def _compute_precio_web(self):
        for event in self:
            if event.precio_reserva_event == 0:
                pago_web = event.precio_total_event - event.descuento_event
            else:
                pago_web = event.precio_reserva_event
            
            event.precio_web = float(pago_web)
    ...
    precio_web = fields.Float(string='Precio Pago Web (por alumno)', digits_compute=dp.get_precision('Product Price'), compute='_compute_precio_web')
