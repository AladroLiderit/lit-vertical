...
class event_event(models.Model):
    ...
    def _compute_total_formacion(self):
        for event in self:
            ...
            #las cantidades se registran sin impuestos, asi que esto lo paramos
            # suma_ivas = 0
            ...
            event.total_formacion = neto_event - total_suplidos
    ...
    precio_total_event = fields.Float(string='Precio total SIN IVA (por alumno)', digits_compute=dp.get_precision('Product Price'))
    precio_reserva_event = fields.Float(string='Precio Reserva SIN IVA (por alumno)', digits_compute=dp.get_precision('Product Price'))
    ...
    def _onchange_reg_agencia(self):
        for e in self:
            if e.reg_agencia:
                ...
                # self.env.context.update( { 'tax_ids': e.tax_ids } )
...
class event_reserva (models.Model):
    ...
    def get_taxes(self):
        for s in self:
            logger.error('Valor de tax_ids en get_taxes %s'%s.event_id.tax_ids)
            tax_ids = []
            if s.event_id.tax_ids:
                for t in s.event_id.tax_ids:
                    tax_ids.append(t.id)
                return tax_ids
            else:
                return self.env['account.tax'].search([('description','like', 'S_IVA0'),('name','ilike', 'exento')]).ids or ''
    ...
    importe_concepto_neto = fields.Float(string='Anticipo SIN IVA', digits_compute=dp.get_precision('Product Price'))
    ...
    tax_ids = fields.Many2many ('account.tax', required=True, string='Tipo de IVA')
    ...
    def _onchange_tax_ids(self):
        if self.tax_ids:
            if self.importe_concepto:       
                ...
                self.importe_impuesto = self.importe_concepto - (self.importe_concepto/(1+amount_tax))
                self.importe_concepto_neto = self.importe_concepto/(1+amount_tax)
    ...
    @api.onchange('pago_name')
    def _onchange_pago_name(self):
        self.tax_ids = [(6,0,self.get_taxes())]
